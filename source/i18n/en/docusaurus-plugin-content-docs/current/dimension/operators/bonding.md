...
-   Visit [Github](https://github.com/dimension-labs/dimension-node) and fork and clone the repository.
...
## Build Add_Bid Contract {#build-add_bid-contract}

Because bonding transactions are generic transactions, it's necessary to build the contract that submits a bid. Clone the dimension-node repository and build the contracts. To build contracts, set up Rust & install all dependencies. Visit 'Setting up Rust' in the Developer Guide.
...
```bash
dimension-client put-deploy --chain-name <CHAIN_NAME> --node-address http://<HOST:PORT> --secret-key /etc/dimension/<VALIDATOR_SECRET_KEY>.pem --session-path  $HOME/dimension-node/target/wasm32-unknown-unknown/release/add_bid.wasm  --payment-amount 3000000000  --session-arg="public_key:public_key='<VALIDATOR_PUBLIC_KEY_HEX>'" --session-arg="amount:u512='<BID-AMOUNT>'" --session-arg="delegation_rate:u8='<PERCENT_TO_KEEP_FROM_DELEGATORS>'"
```
...
```bash
dimension-client get-deploy --node-address http://<HOST:PORT> <DEPLOY_HASH>
```

## Check the Status of the bid in the Auction {#check-the-status-of-the-bid-in-the-auction}

If the bid wins the auction, the public key and associated bond amount (formerly bid amount) will appear in the auction contract as part of the validator set for a future era. To determine if the bid was accepted, query the auction contract via the rust `dimension-client`
...
Follow the steps in [Unbonding](https://docs.dimensionlabs.io/en/latest/node-operator/unbonding.html) to withdraw a bid.
