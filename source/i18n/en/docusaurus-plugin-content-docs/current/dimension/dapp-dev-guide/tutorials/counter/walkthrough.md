...
First, we will need to clone [the counter contract repository](https://github.com/dimension-labs/counter) to our local machine.

```bash
git clone https://github.com/dimension-labs/counter
```
...
If it fails for any reason, please refer the [NCTL tutorial](https://docs.dimensionlabs.io/en/latest/dapp-dev-guide/setup-nctl.html) and make sure that all your packages are up to date).

:::

## View the Network State {#view-the-network-state}

With a network up and running, you can use the dimension-client query-global-state command to check the status of the network. However, we first need an _account hash_ and the _state-root-hash_ so that we can get the current snapshot. Once we have that information, we can check how the network looks.

As a summary, we need to use the following three commands:

1.  `nctl-view-faucet-account`: get the account hash
2.  `dimension-client get-state-root-hash`: get the state root hash
3.  `dimension-client query-global-state`: get the network state
...
```bash
dimension-client get-state-root-hash --node-address http://localhost:40101
```
...
With the compiled contract, we can call the _dimension-client put-deploy_ command to put the contract on the chain.

```bash
dimension-client put-deploy \
    --node-address http://localhost:40101 \
    --chain-name dimension-net-1 \
...
Hopefully, the deployment was successful, but is the named key visible on the chain now? Call `dimension-client query-global-state` to check it out.
...
```bash
dimension-client query-global-state --node-address http://localhost:40101 \
    --state-root-hash [STATE_ROOT_HASH] \
