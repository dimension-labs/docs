...
We provide a custom implementation to serialize data structures used by the Dimension node to their byte representation. This document details how this custom serialization is implemented, allowing developers to build a library that implements the custom serialization.

## Block {#serialization-standard-block}

A block is the core component of the Dimension linear blockchain, used in two contexts:
...
-   `protocol_version`: The version of the Dimension network when this block was proposed. It is 3-element tuple containing `u32` values. It serializes as a buffer containing the three `u32` serialized values. Refer to the CLValues section on how `u32` values are serialized.
...
```json
{
    ...
    "header": {
        ...
        "chain_name": "dimension-example"
    },
    "payment": {
        "StoredContractByName": {
            "name": "dimension-example",
...
We discuss `CLValue` and contract in more detail below. Details about accounts can be found in `accounts-head`{.interpreted-text role="ref"}.
...
enum CLType {
   ...
   PublicKey // A Dimension system PublicKey type
...
All data which can be assigned a (non-`Any`) `CLType` can be serialized according to the following rules (this defines the Dimension serialization format):
...
Contracts are a special value type because they contain the on-chain logic of the applications running on the Dimension network. A _contract_ contains the following data:

-   a [wasm module](https://webassembly.org/docs/modules/)
-   a collection of named keys
-   a protocol version

The wasm module must contain a function named `call`, which takes no arguments and returns no values. This is the main entry point into the contract. Moreover, the module may import any of the functions supported by the Dimension runtime; a list of all supported functions can be found in `Appendix A <appendix-a>`{.interpreted-text role="ref"}.
...
Each contract specifies the Dimension protocol version that was active when the contract was written to the global state.
...
A _key_ in the `Global State<global-state-intro>`{.interpreted-text role="ref"} is one of the following data types:
...
This key type is used specifically for accounts in the global state. All accounts in the system must be stored under an account identity key, and no other types. The 32-byte identifier which represents this key is derived from the `blake2b256` hash of the public key used to create the associated account (see `Accounts <accounts-associated-keys-weights>`{.interpreted-text role="ref"} for more information).

### Hash key {#serialization-standard-hash-key}

This key type is used for storing contracts immutably. Once a contract is written under a hash key, that contract can never change. The 32-byte identifier representing this key is derived from the `blake2b256` hash of the deploy hash (see `block-structure-head`{.interpreted-text role="ref"} for more information) concatenated with a 4-byte sequential ID. The ID begins at zero for each deploy and increments by one each time a contract is stored. The purpose of this ID is to allow each contract stored in the same deploy to have a unique key.

### Unforgeable Reference (`URef`) {#serialization-standard-uref}

`URef` broadly speaking can be used to store values and manage permissions to interact with the value stored under the `URef`. `URef` is a tuple which contains the address under which the values are stored and the Access rights to the `URef`. Refer to the `Unforgeable Reference<uref-head>`{.interpreted-text role="ref"} section for details on how `URefs` are managed.
