...
The Dimension network is permissionless, enabling new validators to join the network and provide additional security to the system. This page outlines the sequence of recommended steps to spin up a validating node and join an existing network.
...
1. Clone the dimension-node repository.
...
To create keys and fund your account, you must setup the dimension command line client. For instructions on setting up the command line client, creating keys and funding your accounts, see [Prerequisites](../workflow/setup.md). The tokens acquired from funding your account are needed to bond your node on to the network and to pay for the bonding transaction.

Generate the keys in the `/etc/dimension/validator_keys` folder using the following command:
...
dimension-client get-block --node-address http://<NODE_IP_ADDRESS>:7777 -b 20

# If jq is installed we can pull it from the JSON return directly with
dimension-client get-block --node-address http://<NODE_IP_ADDRESS>:7777 -b 20 | jq -r .result.block.hash
```
...
For the node to connect to a network, the node needs a set of trusted peers for that network. For [Mainnet](https://dscc.live/), these are listed in the `config.toml` as `known_addresses`. For other networks, locate and update the list to include at least two trusted IP addresses for peers in that network. Here is an [example configuration](https://github.com/dimension-labs/dimension-protocol-release/blob/main/config/config-example.toml). The [dimension-protocol-release](https://github.com/dimension-labs/dimension-protocol-release) repository stores configurations for various environments, which you can also use as examples.
...
The Debian package installs a dimension-node service for systemd. Start the node with:
...
While the node is synchronizing, the `/status` endpoint is available. You will be able to compare this to another node's status endpoint `era_id` and `height` to determine if you are caught up. You will not be able to perform any `dimension-client` calls to your `7777` RPC port until your node is fully caught up.
...
```json
{
  ...
  "round_length": null,
  "next_upgrade": null,
  "build_version": "1.4.3-a44bed1fd-dimension-mainnet",
