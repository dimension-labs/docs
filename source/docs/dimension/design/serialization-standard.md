...
The wasm module must contain a function named `call`, which takes no arguments and returns no values. This is the main entry point into the contract. Moreover, the module may import any of the functions supported by the Dimension runtime; a list of all supported functions can be found in [Appendix A](./appendix.md#appendix-a).
