...
- Since we are deploying to the Dimension Testnet, ensure your [Testnet faucet account](https://testnet.dscc.live/tools/faucet) contains enough DSCC tokens to perform the contract execution. Follow [fund your account](/docs/workflow/setup#fund-your-account) guide to add DSCC tokens to your account
- DSCC tokens are used to pay for the transactions on the Dimension Network. Follow the [transfer tokens](/docs/workflow/token-transfer#2-the-faucet) guide to learn more about token transferring on the Dimension Testnet

## Basic Flows of the Deployment
Here are the basic steps for deploying your contract on the Dimension Network.
<img src={useBaseUrl("/image/tutorials/cep-47/CEP-47-deploy-flow.png")} alt="cep-47-deploy-flow" width="800"/>


### Dimension Repositories

You will be using two Dimension repositories for the deployment process.
-  [dimension-nft-cep47](https://github.com/dimension-labs/dimension-nft-cep47) - a repository containing the implementation of the NFT, a.k.a. CEP-47 smart contract, required utility classes, and corresponding test suite to work with the CEP-47 token.
...
- [dimension-contracts-js-clients](https://github.com/dimension-labs/dimension-contracts-js-clients) - a repository containing a JS client for the CEP-47 contract and other supporting classes to run the client.
    - You will be executing the [install.ts](https://github.com/dimension-labs/dimension-contracts-js-clients/blob/master/e2e/cep47/install.ts) file for the deployment.

## Deploying the Contract

### 1. Preparing the CEP-47 contract repository

Refer to the [contract preparation](prepare.md) step to prepare the [NFT contract](https://github.com/dimension-labs/dimension-nft-cep47) for deployment. This step will make the build environment, create the target location and compile the contract to a .wasm file.
...
The JS client can be used to install the smart contract on the Dimension Network and perform further actions with the contract. We are using the JS client classes to invoke the NFT installation on the network using a pre-defined set of environment variables.

Clone the [dimension-contracts-js-clients](https://github.com/dimension-labs/dimension-contracts-js-clients) repository using the following command.

```
git clone https://github.com/dimension-labs/dimension-contracts-js-clients.git
```
...
1. In the root folder of the newly-cloned repository, copy or rename the sample .env file from *.env.cep47.example* to *.env.cep47*:
  ```bash
    cd dimension-contracts-js-clients
...
2. In the *.env.cep47* file, replace the following values with your own:
    ...
    - `CHAIN_NAME` - Network name, e.g. *dimension* for Mainnet or *dimension-test* for Testnet
...
This section clarifies how the contract deployment happens through the [install.ts](https://github.com/dimension-labs/dimension-contracts-js-clients/blob/master/e2e/cep47/install.ts) file.
