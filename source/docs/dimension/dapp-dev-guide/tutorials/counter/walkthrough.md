...
With a network up and running, you can use the `dimension-client query-global-state` command to check the status of the network. However, we first need an `account hash` and the `state-root-hash` so that we can get the current snapshot. Once we have that information, we can check how the network looks.
...
```bash
dimension-client get-state-root-hash --node-address http://localhost:11101
```
...
With the compiled contract, we can call the `dimension-client put-deploy` command to put the contract on the chain.
...
Hopefully, the deployment was successful. Let us call the `dimension-client query-global-state` command to check if the named key is visible on the chain now.
...
```bash
dimension-client query-global-state --node-address http://localhost:11101 \
    --state-root-hash [STATE_ROOT_HASH] \
