...
This command allows you to query the state of a Dimension network at a given moment in time, which is specified by the _state-root-hash_ described above.
...
-   This contract is then deployed to the network specified by _node-address_ and _chain-name_. By default, NCTL names the chain "dimension-net-1" but this is configurable.
-   The _payment-amount_ is in units of motes (1 nano-DSCC) and is required to pay the transaction fee for the deployment. If it is too small, the transaction will get denied due to insufficient funds.
