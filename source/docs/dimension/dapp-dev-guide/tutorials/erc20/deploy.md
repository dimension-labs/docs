...
- Ensure you have an [Account](https://dimension.labs/docs/workflow/setup#setting-up-an-account), the associated key pair,  and the required amount of tokens to perform the deploy
- Ensure your [account](https://testnet.dscc.live/tools/faucet) contains enough `DSCC Tokens` to perform the execution.
  -DSCC Tokens are used to pay for the transactions on the Dimension Network. Follow the [transfer tokens](https://dimension.labs/docs/workflow/token-transfer#2-the-faucet) guide to learn more about token transferring on the Dimension Testnet
- Install [Dimension client](/dapp-dev-guide/tutorials/counter/setup) to interact with the network

## Basic Flow
Here are the basic steps to deploy the ERC-20 contract on the Dimension Network.
...
```bash
git clone https://github.com/dimension-labs/erc20.git
```
...
Use the [acquire node address](/docs/workflow/setup#acquire-node-address-from-network-peers) section to get a node-ip-address. We use [peers](https://testnet.dscc.live/tools/peers) on Testnet since we are deploying to the Testnet. Select a peer address from the list and do the address format as below,
:::note
...
```bash
dimension-client get-state-root-hash --node-address http://<HOST:PORT>
```
...
```bash
dimension-client account-address --public-key "[PATH_TO_YOUR_KEY]/public_key_hex"
```
...
```
- `NETWORK_NAME`: Use the relevant network name. Here we use '*dimension-test*'
...
- **We currently do not refund any tokens as part of a deploy.**
  ...
  Eg:- If you spend 10 DSCC for the deployment and it only costs 1 DSCC, you will not receive the extra 9 DSCC. Refer to the [computational cost and gas amounts](https://dimension.labs/docs/design/execution-semantics#execution-semantics-gas) for further details
:::
...
```bash
dimension-client get-deploy \
--node-address http://<HOST:PORT> [DEPLOY_HASH]
```

## Querying with Arguments
This step will narrow down the context and check the status of a specific entry point. You will use the details inside the [erc20 contract](https://github.com/dimension-labs/erc20/blob/master/example/erc20-token/src/main.rs) to derive arguments.
:::
...
Use [peers](https://testnet.dscc.live/tools/peers) site to get the node ip address.
...
```bash
dimension-client query-global-state --node-address http://95.216.24.237:7777 \
...
You can see that the name is `DimensionTest` in this example.
```bash
{
