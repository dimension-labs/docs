...
We will review the following three [GitHub testing folders](https://github.com/dimension-labs/erc20/tree/master/testing), which create a testing framework for the Dimension [ERC-20](https://github.com/dimension-labs/erc20) project:
...
To run the tests, issue the following command in the project folder, [erc20](https://github.com/dimension-labs/erc20):
...
The project contains a [Makefile](https://github.com/dimension-labs/erc20/blob/master/Makefile), which is a custom build script that compiles the contract before running tests in _release_ mode. Then, the script copies the `contract.wasm` file to the [tests/wasm](https://github.com/dimension-labs/erc20/tree/master/testing/tests/wasm) directory. In practice, you only need to run the `make test` command during development.

## Configuring the Test Package {#configuring-the-test-package}

In this project, we define a `tests` package using the [tests/Cargo.toml](https://github.com/dimension-labs/erc20/blob/master/testing/tests/Cargo.toml) file.
...
```bash
    ...
    [dependencies]
    dimension-types = "1.3.2"
    dimension-engine-test-support = "1.3.2"
    dimension-execution-engine = "1.3.2"
    once_cell = "1.8.0"
...
In Github, you will find an [example](https://github.com/dimension-labs/erc20/tree/master/example) containing a Dimension ERC-20 [contract](https://github.com/dimension-labs/erc20/blob/master/example/erc20-token/src/main.rs) implementation with the corresponding [tests](https://github.com/dimension-labs/erc20/tree/master/example/erc20-tests/src). The tests follow this sequence:
...
The [TestFixture](https://github.com/dimension-labs/erc20/blob/master/example/erc20-tests/src/test_fixture.rs) accomplishes these steps by simulating a real-world deployment (storing the contract in the blockchain) and invoking methods in the contract.

### Setting up the testing context {#setting-up-the-testing-context}

The code in the [TestFixture](https://github.com/dimension-labs/erc20/blob/master/example/erc20-tests/src/test_fixture.rs) initializes the blockchain's [global state](../../../glossary/G.md#global-state) with all the data and methods a smart contract needs.

Below is a subset of the required constants for this project. For the most up-to-date version of the code, visit [GitHub](https://github.com/dimension-labs/erc20).

```rust

    // File https://github.com/dimension-labs/erc20/blob/master/example/erc20-tests/src/test_fixture.rs

    use dimension_engine_test_support::{Code, SessionBuilder, TestContext, TestContextBuilder};
    use dimension_erc20::constants as consts;
    use dimension_types::{
        account::AccountHash,
...
The full and most recent code implementation is available on [GitHub](https://github.com/dimension-labs/erc20/blob/master/example/erc20-tests/src/test_fixture.rs).
...
The next code sample shows how to invoke one of the methods in the contract. The rest of the methods are available on [GitHub](https://github.com/dimension-labs/erc20/blob/master/example/erc20-tests/src/test_fixture.rs).
...
Now that we have a testing context, we can use it to create unit tests in a file called [integration_tests.rs](https://github.com/dimension-labs/erc20/blob/master/example/erc20-tests/src/integration_tests.rs). The unit tests verify the contract code by invoking the functions defined in the [test_fixture.rs](https://github.com/dimension-labs/erc20/blob/master/example/erc20-tests/src/test_fixture.rs) file.

The example below shows one of the example tests. Visit [GitHub](https://github.com/dimension-labs/erc20/blob/master/example/erc20-tests/src/integration_tests.rs) to find all the available tests.

```rust

    // File https://github.com/dimension-labs/erc20/blob/master/example/erc20-tests/src/integration_tests.rs

    use dimension_types::{Key, U256};
...
We have configured the [lib.rs](https://github.com/dimension-labs/erc20/blob/master/testing/tests/src/lib.rs) file to run the example integration tests via the `make test` command:
...
To run the tests, navigate to the parent [erc20 directory](https://github.com/dimension-labs/erc20) and run the command:
