...
- The `chain-name` attribute, is `dimension-test` for Testnet and `dimension` for Mainnet.
...
To create accounts on the Dimension blockchain, you need to generate keys and fund your accounts. For more information see [Account and Cryptographic Keys](../../keys.md).
...
```bash
dimension-client get-deploy <deploy hash> \
--node-address http://[NODE_IP_ADDRESS]:7777
```
...
```
dimension-client get-account-info \
...
```bash
dimension-client get-state-root-hash --node-address http://[NODE_IP_ADDRESS]:7777
```
