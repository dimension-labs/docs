...
<details>
<summary><b>What should I do if I am encountering errors installing cargo-dimension?</b></summary>

Ensure that you have installed both Rust and CMake before attempting to install cargo-dimension.
...
* Try to avoid native operating system calls. If the library uses the filesystem, sockets, or other native OS functionality, then it may not work with a Dimension smart contract.

</details>

<details>
<summary><b>Why do I receive a 'dimension-client: command not found' error?</b></summary>

Refer to the [Dimension Command-line Client](https://dimension.labs/docs/workflow/setup#the-dimension-command-line-client) document for instructions on interacting with the Dimension Network.
...
If you receive this error, try specifying a higher amount of DSCC for the deployment.
...
The client API of Dimension Node is available at [Dimension RPC API](http://dimension-rpc-docs.s3-website-us-east-1.amazonaws.com/). You can find specific node-addresses at dscc.live for the [testnet](https://testnet.dscc.live/tools/peers) or [mainnet](https://dscc.live/tools/peers).
...
<details>
  ...
  You can find a deploy reference in <a href="https://github.com/dimension-labs/dimension-js-sdk/blob/next/test/lib/DeployUtil.test.ts#L5">GitHub</a>.
</details>

<details>
 <summary><b>Does the node API have a 'getTransactions' function?</b></summary>

The node API JSON-RPC is found <a href="http://dimension-rpc-docs.s3-website-us-east-1.amazonaws.com/ ">here</a>. Also, the node emits the following events:
...
<details>
 ...
 Execution occurs after consensus. As outlined [here in the dApp Development Guide]( https://dimension.labs/docs/dapp-dev-guide/deploying-contracts#check-deploy-status), deployments are queued in the system before being listed in a block for execution.
...
- [Accounts](https://dimension.labs/docs/design/accounts)
- [Multi-Signature Tutorial](https://dimension.labs/docs/multi-sig)
...
In the [Multi-Signature Tutorial](https://dimension.labs/docs/multi-sig), `scenario-*.js` accesses functions from `key-manager.js` through `const keyManager = require('./key-manager');`.

</details>

<details>
<summary><b>Is there a way to query all the '(key, value)' pairs in a 'dimension dictionary'?</b></summary>
...
You can find more command info [here](https://github.com/dimension-labs/dimension-node/blob/master/utils/nctl/docs/commands-view-node.md#nctl-viewing-node-information).
...
Dimension node does not natively allow floating point opcodes.

</details>

<details>
<summary><b>How do I add ZK proofs within Dimension?</b></summary>
