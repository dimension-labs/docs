# Deploying an ERC-20 Compliant Token on Dimension

On the Dimension Network, it is possible to create smart contracts that emulate ERC-20 tokens on Ethereum. These tokens have all of the capabilities of traditional ERC-20 tokens, allowing one to approve, transfer, inquire the balance of, etc. By following this guide, you'll be able to create your own ERC-20 token and deploy it to the Dimension Network.

## Prerequisites

- [Set up an account](https://dimension.labs/docs/workflow/setup#setting-up-an-account).
- [Fund your account](https://dimension.labs/docs/workflow/setup#fund-your-account).
...
Now clone the prewritten [JavaScript deployer project](https://github.com/dimension-labs/dimension-erc20-js-interface), so you don't have to reinvent the wheel. This project includes a precompiled ERC-20 contract with only basic functionality. If you'd like to write your own ERC-20 contract with custom logic, please follow the instructions [here](https://dimension.labs/docs/writing-contracts). Execute this command in your home directory:

`git clone https://github.com/dimension-labs/dimension-erc20-js-interface.git`

Change your working directory to the project directory:

`cd dimension-erc20-js-interface`
...
const NETWORK_NAME = "dimension-test";
```
...
* `NODE_ADDRESS`: The validator node used to submit the deploy. The address listed directs to a valid online node, but this may change in the future. If this node does not respond, you can select another online peer from the list [here](https://testnet.dscc.live/tools/peers). Note that you'll need to replace the port with `7777` for most nodes and add `/rpc` to the end of the address.
* `NETWORK_NAME`: The name of the network to which you'll be deploying. By default, we have the Dimension Testnet specified as `"dimension-test"`. To deploy on the Mainnet, you may change this to `"dimension"`.
...
You can send ERC-20 compliant tokens on the Dimension Network as per the ERC specification. We'll use the same  `erc20iface` script to execute the transfer deployments.
...
Thank you for reading Dimension's ERC-20 deployment guide. Next, you can learn how to write your own [ERC-20 smart contracts](https://dimension.labs/docs/erc20), and use [Dimension's JavaScript SDK](https://dimension.labs/docs/dapp-dev-guide/sdk/script-sdk).
