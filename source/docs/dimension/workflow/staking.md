...
1.  Using Chrome or a Chromium-based browser like Brave, navigate to a block explorer on the Mainnet, (for this example we are using [dscc.live](https://dscc.live/)), and click on the **Sign-in** menu.
...
If you are using the DimensionLabs Signer for the first time, follow these steps to create an account and **download the account's secret key**:

1.  Click **CREATE ACCOUNT** in the Signer.
2.  Give your account a name. In this tutorial, we will use the name _My-DSCC_.
3.  Select an Algorithm using the dropdown menu. If you need help, review the article on [Working with Cryptographic Keys](https://dimension.labs/docs/dapp-dev-guide/keys).
...
Using the active account in the Signer tool, connect to the Dimension blockchain by clicking on the **DISCONNECTED** button to toggle the connection. For this example, we are using the [dscc.live](https://dscc.live/) block explorer.
...
If you need to transfer your DSCC tokens from an exchange, you will need your **public key** from the account page.
